...
import { PRICING, STARTER_PRICING } from "@/lib/constants";
...
const PricingSection = () => {
  const handleCTA = () => trackAndRedirect();
  const handleStarter = () => trackAndRedirectStarter();
  ...
  return (
    <section id="pricing" className="py-24 sm:py-32 px-4 bg-gradient-section">
      <div className="max-w-6xl mx-auto text-center">
        ...
        {/* Two-card pricing grid: Starter (left, ~70%) + Vault (right, dominant) */}
        <div className="grid lg:grid-cols-[minmax(0,0.7fr)_minmax(0,1fr)] gap-6 lg:gap-8 items-stretch relative">
          {/* ── STARTER CARD ── */}
          <div className="reveal relative rounded-2xl border border-border bg-card/60 p-6 sm:p-8 flex flex-col text-left">
            <div className="mb-4">
              <p className="text-[11px] uppercase tracking-widest text-muted-foreground font-bold mb-1">
                The Starter
              </p>
              <div className="flex items-baseline gap-2">
                <span className="text-4xl sm:text-5xl font-black text-foreground">${STARTER_PRICING.price}</span>
                <span className="text-xs text-muted-foreground">one-time</span>
              </div>
            </div>
            <ul className="space-y-2 text-sm mb-6 flex-1">
              {starterInclusions.map((item) => (
            ...
            <Button
              ...
              variant="outline"
              size="lg"
              className="w-full border-[hsl(var(--gold))]/40 text-foreground hover:bg-[hsl(var(--gold))]/10 hover:text-foreground hover:border-[hsl(var(--gold))]/70 font-bold"
            >
              Start with Starter — ${STARTER_PRICING.price}
            </Button>
          </div>

          {/* "Most people skip this →" arrow label between cards (desktop only) */}
          <div className="hidden lg:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-20 pointer-events-none">
            <span className="bg-[hsl(var(--gold))]/15 border border-[hsl(var(--gold))]/40 text-[hsl(var(--gold))] text-[11px] font-bold uppercase tracking-wider rounded-full px-3 py-1.5 whitespace-nowrap shadow-lg">
              Most people skip this →
            </span>
          </div>

          {/* ── VAULT CARD (existing, dominant) ── */}
          <div className="reveal reveal-delay-1 relative">
            ...
            <div className="relative rounded-2xl bg-[hsl(var(--card))] p-8 sm:p-12 z-10">
              <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-cta text-[hsl(var(--primary-foreground))] px-6 py-1.5 rounded-full text-sm font-bold badge-shine whitespace-nowrap">
                Save ${(PRICING.original - PRICING.sale).toFixed(0)} ({PRICING.discount}% OFF)
              </div>

              <div className="mb-8 mt-4">
                <p className="text-[11px] uppercase tracking-widest text-[hsl(var(--gold))] font-bold mb-2">
                  The Vault — Best Value
                </p>
              ...
              <div className="flex items-center justify-center gap-4 mt-6 text-xs text-[hsl(var(--muted-foreground))] flex-wrap">
                <span className="flex items-center gap-1.5">
                  <MoneyBackSeal size={28} />
                  30-Day Money-Back Guarantee
                </span>
                <span className="flex items-center gap-1.5">
